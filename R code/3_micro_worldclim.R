#############Micro_Global from WorldClim 2.1###############
...
micro_world <- function(
  ...
  if(errors==0){ # continue
    ...
    clmfolder<-"E:/QT/NicheMapR/Data/global_climate_JZW/"
    
    gcfuture<-paste(.libPaths()[1],"/gcfuture.rda",sep="")
    if(file.exists(gcfuture)==FALSE){
      climate<-clmfolder
      if(file.exists(paste0(climate,"/worldclim_",time,".nc"))==FALSE){
        message("You don't appear to have the global future climate data set - \n run function get.global.climate(folder = 'folder you want to put it in') .....\n exiting function micro_global")
    ...
    global_climate_future <- raster::brick(paste0(climate,"/worldclim_",time,".nc"))
    CLIMATE <- raster::extract(global_climate_future, x)
    ...
    if(is.na(RAINFALL[1])){
      ...
      CLIMATE <- raster::extract(global_climate_future, cbind(140, -35))
      CLIMATE[2:97] <- 0
      ALTT<-as.numeric(CLIMATE[, 1])
      delta_elev <- 0
      if(is.na(elev) == FALSE){ # check if user-specified elevation
    ...
    if(runmoist == 0){
      ...
      soilmoisture <- suppressWarnings(raster::brick(paste(clmfolder, "/soilw.mon.ltm.v2.nc", sep = "")))
